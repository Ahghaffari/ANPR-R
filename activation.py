...
class SerialNumber:
    ...
    def __init__(self):
        self._library = ctypes.CDLL("libs/Activator.dll")
    ...
    def check_activation_status(self, type):
        ...
        key = self.activation_result.key
        ...
        try:
            f = Fernet(key)
            result = f.decrypt_at_time(token, current_time=int(time.time()), ttl=10).decode("utf-8")
            if result == uuid:
